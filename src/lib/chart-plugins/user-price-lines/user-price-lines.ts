...
class UserPriceLinesPaneRenderer implements ISeriesPrimitivePaneRenderer {
  ...
  draw(target: CanvasRenderingTarget2D) {
    ...
    target.useBitmapCoordinateSpace((scope) => {
      ...
      const xPos = positionsBox(
        this._data.rightX - width,
        this._data.rightX - 1,
      ...
      ctx.translate(
        xPos.position + 3 * scope.horizontalPixelRatio,
        yPos.position + 3 * scope.verticalPixelRatio,
      );
...
class UserPriceLinesPaneView extends UserPriceLineDataBase implements ISeriesPrimitivePaneView {
