...
import { BrushRange, BrushableAreaSeriesOptions, BrushableAreaStyle } from "./options";
  ...
  _drawImpl(
    ...
    const bars: BrushableAreaBarItem[] = this._data.bars.map((bar) => {
      return {
        ...
        y: priceToCoordinate(bar.originalData.value)! * renderingScope.verticalPixelRatio,
      };
    });
    ...
    const getRangeStyle = (index: number): BrushableAreaStyle => {
      ...
      const foundRange = options.brushRanges.findIndex((brushRange: BrushRange) => {
    ...
    const rangeStyles: BrushableAreaStyle[] = new Array(this._data.visibleRange.to);
    ...
    for (let i = this._data.visibleRange.from + 1; i < this._data.visibleRange.to; i++) {
      ...
      const rangeStyleWillChange =
        i === this._data.visibleRange.to - 1 ? true : rangeStyles[i + 1] !== rangeStyle;
      ...
      if (rangeStyleWillChange) {
        ...
        ctx.lineWidth = rangeStyle.lineWidth * renderingScope.verticalPixelRatio;
        ctx.stroke();
      }
      previousPosition = [bar.x, bar.y];
    }
