...
export class PrettyHistogramSeriesRenderer<
  HorzScaleItem,
  TData extends PrettyHistogramData<HorzScaleItem>,
> implements ICustomSeriesPaneRenderer
{
  ...
  public draw(target: CanvasRenderingTarget2D, priceConverter: PriceToCoordinateConverter): void {
  ...
  private _drawImpl(
    ...
    const bars: PrettyHistogramBarItem[] = this._data.bars.map(
      (bar: CustomBarItemData<HorzScaleItem, TData>) => {
        return {
    ...
    const width = Math.max(
      1,
      Math.round(
        0.01 * options.widthPercent * this._data.barSpacing * renderingScope.horizontalPixelRatio,
      ),
    );
    ...
    bars
      ...
      .forEach((item: PrettyHistogramBarItem) => {
