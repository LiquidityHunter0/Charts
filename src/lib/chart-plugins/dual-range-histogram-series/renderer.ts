...
import { ColumnPosition, calculateColumnPositionsInPlace } from "../helpers/dimensions/columns";
import { positionsBox } from "../helpers/dimensions/positions";
import { drawRoundRectWithBorder, LeftTopRightTopRightBottomLeftBottomRadii } from "./canvas-utils";
  ...
  _drawImpl(
    ...
    for (let i = this._data.visibleRange.from; i < this._data.visibleRange.to; i++) {
      ...
      group.ys.forEach((y, index) => {
        ...
        const columnPosition = positionsBox(zeroY, zeroY - y, renderingScope.verticalPixelRatio);
        const radius =
