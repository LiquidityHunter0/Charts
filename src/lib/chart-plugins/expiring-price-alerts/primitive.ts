  ...
  update() {
    ...
    if (ts) {
      for (const alert of this._source._alerts.values()) {
        ...
        let startX: number | null = ts.timeToCoordinate(alert.start as Time) as number | null;
        let endX: number | null = ts.timeToCoordinate(alert.end as Time) as number | null;
